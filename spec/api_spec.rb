...
describe 'Test Phonebook Web API' do
      include Rack::Test::Methods

      before do
      # Wipe database before each test
      Dir.glob("#{Phonebook::STORE_DIR}/*.txt").each { |filename| FileUtils.rm(filename) }
      end
            ...
            it 'HAPPY: should be able to get list of all documents' do
                  Phonebook::Contant.new(DATA[0]).save
                  Phonebook::Contant.new(DATA[1]).save
            ...
            it 'HAPPY: should be able to get details of a single document' do
                  Phonebook::Contant.new(DATA[1]).save
                  id = Dir.glob("#{Phonebook::STORE_DIR}/*.txt").first.split(%r{[/.]})[3]
            ...
            it 'HAPPY: should be able to create new documents' do
                  ...
                  post 'api/v1/contacts', DATA[1].to_json, req_header
