...
class AuthToken
  ...
  def self.create(payload, scope = AuthScope.new, expiration = ONE_WEEK)
    tokenize('payload' => payload, 'scope' => scope, 'exp' => expires(expiration))
  end
  ...
  def self.tokenize(message)
    ...
    Base64.urlsafe_encode64(base_encrypt(message.to_json))
  end
  ...
  def self.detokenize(ciphertext64)
    ...
    JSON.parse(base_decrypt(Base64.urlsafe_decode64(ciphertext64)))
